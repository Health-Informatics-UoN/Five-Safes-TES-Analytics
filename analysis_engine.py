...
class AnalysisEngine:
    ...
    def __init__(self, token: Optional[str] = None, project: Optional[str] = None):
        """
        ...
        if token is None:
            ...
            if not token:
                raise ValueError(
                    "TRE_FX_TOKEN environment variable is required when token parameter is not provided"
                )
        self.token = token
        ...
        if project is None:
            ...
            if not project:
                raise ValueError(
                    "TRE_FX_PROJECT environment variable is required when project parameter is not provided"
                )
        ...
        self.tes_client = TESClient(
            base_url=str(settings.tes_base_url),
    ...
    def run_analysis(
        ...
        if bucket is None:
            ...
            #You've said that this is obligatory
            
        if tres is None:
            tres = settings.tre_fx_tres
        ...
        try:
            ...
            submission_task, n_results = self.tes_client.generate_submission_template(
                name=task_name, query=complete_query, tres=tres, project=self.project, output_bucket=bucket,
            )
            ...
            results_paths = [
                f"{int(task_id) + i + 1}/output.csv" for i in range(n_results)
            ]


            ## use polling engine to collect results
            polling_engine = polling.Polling(
                self.tes_client, self.minio_client, task_id
            )
