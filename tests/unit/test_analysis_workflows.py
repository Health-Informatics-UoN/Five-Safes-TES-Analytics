...
class TestAnalysisCompatibility:
    ...
    def runner(self):
        ...
        analysis_runner = AnalysisRunner(tes_client, "test_project")
        return analysis_runner

    @patch("five_safes_tes_analytics.clients.analytics_tes_client.AnalyticsTES")
    @patch("five_safes_tes_analytics.runners.analysis_orchestrator.MinIOClient")
    def test_incompatible_analysis_on_same_data(
        self, mock_minio, mock_tes_class, mock_submission_api_session, runner
    ):
        """Test what happens when running incompatible analyses on the same data."""
        # Mock TES client
        mock_tes_instance = Mock()
        mock_tes_instance.generate_submission_template.return_value = (
            {"task": "data"},
            2,
        )
        mock_tes_instance.submit_task.return_value = {"id": "123"}
        # Mock get_task_status to return a proper dictionary with status
        mock_tes_instance.get_task_status.return_value = {
            "status": 11,
        ...
        mock_orchestrator._submit_and_collect_results.return_value = ("123", raw_data)
        ...
        with patch(
            ...
            with patch(
                "five_safes_tes_analytics.runners.analysis_runner.AnalysisOrchestrator",
                ...
                def mock_analyze_data(input_data, analysis_type):
                    ...
                    if analysis_type == "mean":
                        ...
                        analysis_runner.statistical_analyzer.analysis_classes[
                            "mean"
                        ].aggregated_data = {"n": 25, "total": 250}
                        return 10.0  # mean result
                    else:
                        raise Exception("Incompatible analysis")

                analysis_runner.statistical_analyzer.analyze_data = Mock(
                    side_effect=mock_analyze_data
                )
    ...
    @patch("five_safes_tes_analytics.clients.analytics_tes_client.AnalyticsTES")
    @patch("five_safes_tes_analytics.runners.analysis_orchestrator.MinIOClient")
    def test_compatible_analysis_on_same_data(self, mock_minio, mock_tes_class, mock_submission_api_session, runner):
        ...
        mock_orchestrator = Mock()
        mock_orchestrator.setup_analysis.return_value = ("metadata_task", None, "test-bucket", ['TRE1', 'TRE2'])
        mock_orchestrator._submit_and_collect_results.return_value = ("123", variance_raw)
        mock_orchestrator.tres = ['TRE1', 'TRE2']
        ...
        with patch(
            ...
            with patch(
                ...
                analysis_runner = AnalysisRunner(mock_tes_instance, "test_project")
                ...
                analysis_runner.data_processor.aggregate_data = Mock(
                    return_value=aggregated_variance_data
                )
                ...
                def mock_analyze_data(input_data, analysis_type):
                    if analysis_type == "variance":
                        analysis_runner.statistical_analyzer.analysis_classes[
                            "variance"
                        ].aggregated_data = {"n": 300, "sum_x2": 22000.75, "total": 1350.5}
                        return 15.25
                    else:
                        raise Exception("Incompatible analysis")
    ...
    @patch("five_safes_tes_analytics.runners.analysis_orchestrator.MinIOClient")
    def test_dictionary_based_analysis(self, mock_minio, mock_submission_api_session, runner):
        """
        ...
        mock_tes_client.get_task_status.return_value = {
            "status": 11,
            "description": "Completed",
        }
        ...
        mock_minio.get_object.return_value = "n,sum_x2,total\n10,1000,100"

        
        with patch("five_safes_tes_analytics.runners.analysis_runner.SubmissionAPISession", return_value=mock_submission_api_session), \
            patch("five_safes_tes_analytics.runners.analysis_runner.AnalysisOrchestrator", return_value=mock_orchestrator): 
                runner = AnalysisRunner(mock_tes_client, "test_project")
                # Mock data processor to return raw data that will be processed by analysis class
                ...
                def mock_analyze_data(input_data, analysis_type):
                    ...
                    if analysis_type == "variance":
                        ...
                        runner.statistical_analyzer.analysis_classes[
                            "variance"
                        ].aggregated_data = {"n": 10, "sum_x2": 1000, "total": 100}
                ...
                runner.tes_client.generate_submission_template = Mock(
                    return_value=({"task": "data"}, 1)
                )
                runner.tes_client.submit_task = Mock(
                    return_value={"id": "123"}
                )
                # Mock get_task_status to return completed status
                runner.tes_client.get_task_status = Mock(
                    return_value={"status": 11, "description": "Completed"}
                )
