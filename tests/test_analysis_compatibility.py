...
class TestAnalysisCompatibility:
    ...
    def test_incompatible_analysis_on_same_data(self, mock_minio, mock_tes, engine):
        ...
        def mock_analyze_data(input_data, analysis_type):
            ...
            if analysis_type == "mean":
                ...
                engine.statistical_analyzer.analysis_classes["mean"].aggregated_data = {
                    "n": 25,
                    "total": 250,
                }
        ...
        result1 = engine.run_analysis("mean", user_query, ["TRE1", "TRE2"])
    ...
    def test_compatible_analysis_on_same_data(self, mock_minio, mock_tes, engine):
        ...
        # Mock MinIO client
        mock_minio_instance = Mock()
        mock_minio_instance.get_object.return_value = (
        ...
        def mock_analyze_data(input_data, analysis_type):
            ...
            if analysis_type == "chi_squared_scipy":
                ...
                engine.statistical_analyzer.analysis_classes[
                    "chi_squared_scipy"
                ].aggregated_data = {"contingency_table": contingency_table}
        ...
        result1 = engine.run_analysis("chi_squared_scipy", user_query, ["TRE1", "TRE2"])
        ...
        result2 = real_analyzer.analyze_data(
            engine.aggregated_data["contingency_table"], "chi_squared_manual"
        )
    ...
    def test_dictionary_based_analysis(self, engine):
        ...
        engine.tes_client.generate_submission_template = Mock(
        ...
        engine.tes_client.get_task_status = Mock(
            return_value={"status": 11, "description": "Completed"}
        )
        engine.minio_client.get_object = Mock(
        ...
        user_query = "SELECT value_as_number FROM measurement WHERE concept_id = 123"
        result = engine.run_analysis("variance", user_query, ["TRE1"])
