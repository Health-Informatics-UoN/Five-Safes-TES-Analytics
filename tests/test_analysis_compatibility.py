...
class TestAnalysisCompatibility:
    ...
    def test_incompatible_analysis_on_same_data(
        self, mock_minio, mock_tes_class, runner
    ):
    ...
    def test_compatible_analysis_on_same_data(self, mock_minio, mock_tes_class, runner):
        ...
        analysis_runner = AnalysisRunner(
            mock_tes_instance, "test_token", "test_project"
        )
    ...
    def test_dictionary_based_analysis(self, runner):
        ...
        runner.analysis_orchestrator.tes_client.generate_submission_template = Mock(
            return_value=({"task": "data"}, 1)
        )
        runner.analysis_orchestrator.tes_client.submit_task = Mock(
            return_value={"id": "123"}
        )
        # Mock get_task_status to return completed status
        runner.analysis_orchestrator.tes_client.get_task_status = Mock(
            return_value={"status": 11, "description": "Completed"}
        )
        runner.analysis_orchestrator.minio_client.get_object = Mock(
            return_value="n,sum_x2,total\n10,1000,100"
        )
